        ...
        <table class="group-table">
            ...
            <tbody>
                <tr v-for="team in group.teams" :key="team.code">
                    ...
                    <td class="hide-small">{{ loaded_matches ? team.group_stats.wins : '?' }}</td>
                    <td class="hide-small">{{ loaded_matches ? team.group_stats.draws : '?' }}</td>
                    <td class="hide-small">{{ loaded_matches ? team.group_stats.losses : '?' }}</td>
                    <td class="hide-small">
                        <template v-if="loaded_matches">
                            <template v-if="team.group_stats.get_goal_difference() >= 0">+</template>{{ team.group_stats.get_goal_difference() }}
                        </template>
                        <template v-else>
                            ?
                        </template>
