        ...
        <template v-if="match">
            ...
            <div v-if="show_penalties">
                <small class="in-progress-penalties">
                    <strong>{{ match.home_team.penalties }} – {{ match.away_team.penalties }}</strong> on penalties
                </small>
            </div>
...
export default {
    ...
    computed: {
        ...
        show_penalties: function() {
            return this.show_scores && (this.match.home_team.penalties + this.match.away_team.penalties) > 0;
        },
