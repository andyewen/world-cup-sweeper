<template>
  <div class="container">
    ...
    <div class="match-day-selector">
      <button
        @click="select_previous_date"
        :disabled="!can_select_previous_date"
      >
        &lt;
      </button>
    ...
    <div class="matches-container">
      <template
        v-if="matches_grouped_by_date && matches_grouped_by_date[selected_date]"
      >
        <MatchLine
          v-for="match in matches_grouped_by_date[selected_date]"
          :key="match.fifa_id"
          :match="match"
      ...
      <template v-else-if="!matches_grouped_by_date"> loading... </template>
...
export default {
  ...
  methods: {
    ...
    select_previous_date: function () {
      if (this.can_select_previous_date)
        this.selected_date = this.previous_date;
    },
  },
  computed: {
    ...
    top_three_teams: function () {
      ...
      return [
        ...
        final_match.home_team.country == final_match.winner_code
          ? final_match.away_team.country
          : final_match.home_team.country,
    ...
    people_with_teams_and_stats: function () {
      ...
      people.forEach(function (person) {
        person.competing_team_count = person.teams.reduce(function (
          accum,
          team
        ) {
        ...
        0);
        person.total_tournament_wins = person.teams.reduce(function (
          accumulator,
          current
        ) {
    ...
    unallocated_teams: function () {
      var self = this;
      var allocated_team_codes = self.people
        .map(function (person) {
          return person.teams;
        })
        .reduce(function (a, b) {
    ...
    groups: function () {
      ...
      groups.forEach(function (group) {
        group.teams.sort(function (a, b) {
          ...
          if (a.group_stats.goals_scored > b.group_stats.goals_scored)
            return -1;
      ...
      self.groups.forEach(function (group) {
        var group_future_matches = self.group_matches.filter(function (match) {
          return (
            ...
            self.teams_by_code[match.home_team.country].group === group.code
          );
        });
        ...
        if (group_future_matches.length) {
          ...
          var possible_group_match_outcomes = product(
            ["H", "A", "D"],
            group_future_matches.length
          );
    ...
    can_select_next_date: function () {
      return (
        this.matches_grouped_by_date && this.next_date <= this.date_range.max
      );
    },
    can_select_previous_date: function () {
      return (
        this.matches_grouped_by_date &&
        this.previous_date >= this.date_range.min
      );
    },
